...
В консоль браузера обе версии печатают дату публикации — её отдаёт GitHub Pages
в заголовке `Last-Modified` (`document.lastModified`), сборка для этого не нужна.
Точный коммит по требованию: `nazook.latest()` дёргает GitHub API и печатает
